    ...
    describe ".timestamp" do
      ...
      context "GIVEN no time, nor format" do
        it "returns the expected String" do
          value(subject.timestamp).must_equal("12340506123456")
        end
      end

      context "GIVEN a time" do
        it "returns the expected String" do
          value(subject.timestamp(Time.new(1234, 5, 6, 12, 34, 56))).must_equal(
            "12340506123456")
        end
      end
      ...
      context "GIVEN a format Symbol" do
        it "returns the expected String" do
          value(subject.timestamp(format: :long)).must_match(
      ...
      context "GIVEN a format String" do
        it "returns the expected String" do
          value(subject.timestamp(format: "%Y%m%d%H%M%S")).must_equal(
            "12340506123456")
        end
      end
    end
